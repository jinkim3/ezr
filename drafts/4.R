...
shiny_analysis_tool <- function(
  ...
  num_of_sigfigs = 3,
  shiny_list_max = 5000,
  ezr_saved_analysis_file_name = "ezr_saved_analysis",
  ezr_analysis_history_file_name = "ezr_analysis_history") {
  ...
  ezr_analysis_history <-
    fread(ezr_analysis_history_file_name)
  ...
  if(is.null(ezr_analysis_history)) {
    ...
    fwrite(
      ezr_analysis_history,
      file = ezr_analysis_history_file_name)
    paste_message("The following file was created: ",
                  ezr_analysis_history_file_name)
  }
  ...
  server <- function(input, output, session) {
    ...
    reactive_dt$saved_analysis <- import_data(
      ezr_saved_analysis_file_name, open_file_if_missing = F)
    reactive_dt$run_analysis <- import_data(
      ezr_analysis_history_file_name, open_file_if_missing = F)
    ...
    observeEvent(input$delete_saved_btn, {
      ...
      output_to_csv(
        ezr_saved_analysis,
        gsub(".csv", "", ezr_saved_analysis_file_name),
        timestamp = F)
    ...
    observeEvent(input$run_btn, {
      ...
      output_to_csv(
        run_analysis_dt,
        gsub(".csv", "", ezr_analysis_history_file_name),
        timestamp = F)
      # save again
