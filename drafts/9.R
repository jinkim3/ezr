...
import_package <- function(package) {
  if(eval(parse(text=paste0(
    "require(", toString(substitute(package)), ") == F")))){
    eval(parse(text=paste0(
      "install.packages(",
      deparse(toString(substitute(package))), ", dependencies = TRUE",
      ", repos = 'https://cloud.r-project.org')")))
  }
  eval(parse(text=paste0(
    "library(", deparse(toString(substitute(package))), ")")))
}

# import data
import_data <- function(
  file_name, sheet_index, skip_rows,
  clean_data_from_qualtrics,
  shark_screener_pass_only,
  read_as_data_table,
  convert_to_numeric_if_all_values_in_the_vector_are_numeric = T,
  replace_blanks_with_na_in_character_vectors,
  open_file_if_missing = NULL,
  optional_arguments) {
  # import packages
  import_package(data.table)
  # choose the file if necessary
  if(missing(file_name)) {
    message("You have not entered a file name.")
    message("Please choose the file yourself.")
    if(is.null(open_file_if_missing)) {
      path_1 <- file.choose()
    } else if(open_file_if_missing == F) {
      message("The file is missing.  Nothing was imported.")
      return()
    }
  } else {
    message(paste0("You have chosen to open the following file: ", file_name))
    # This function requires RStudio
    if(require("rstudioapi") == F) {install.packages("rstudioapi", dependencies = T)}
    if(.Platform$OS.type == "unix") {
      path_1 <- paste0(dirname(rstudioapi::getActiveDocumentContext()$path),
                       "/", file_name)
    } else {
      path_1 <- paste0(gsub("/", "\\\\", dirname(rstudioapi::getActiveDocumentContext()$path)),
                       "\\", file_name)
    }
    if(file.exists(path_1)) {
      message("Good! The file exists.")
    } else {
      if(is.null(open_file_if_missing)) {
        message("The file does not exist. Please choose the file yourself.")
        path_1 <- file.choose()
      } else if(open_file_if_missing == F) {
        message("The file is missing.  Nothing was imported.")
        return()
  ...
  cat(paste0(path_1,"\n"))
  # get file extension
  file_extension_length <- nchar(path_1) - regexpr("\\.[^\\.]*$", path_1)
  file_extension <- substr(path_1,
                           nchar(path_1) - file_extension_length + 1,
                           nchar(path_1))
  if(file_extension == "csv") {
    read_data <- fread(path_1)
  }
  if(file_extension == "xlsx") {
    import_package(readxl)
    read_data <- read_excel(path_1)
    message("The readxl package was used to read the data.")
  }
  if(!file_extension %in% c("csv", "xlsx")) {
    message("The file type is not supported. Currently only the follwing file types are supported: csv, xlsx")
  }
  # report progress
  message("The file above has been imported.")
  # options
  if(!missing(clean_data_from_qualtrics)) {
    if(clean_data_from_qualtrics) {
      read_data <- clean_data_from_qualtrics_v2(read_data)
    }
  }
  if(!missing(shark_screener_pass_only)) {
    if(shark_screener_pass_only) {
      read_data <- shark_screener_pass_only(read_data)
    }
  }
  # replace blanks with na in character vectors
  if(missing(replace_blanks_with_na_in_character_vectors)) {
    replace_blanks_with_na_in_character_vectors <- T
  }
  if(replace_blanks_with_na_in_character_vectors) {
    read_data <-
      replace_blanks_with_na_in_a_character_vector(
        dataset = read_data)
  }
  # convert to numeric
  if(convert_to_numeric_if_all_values_in_the_vector_are_numeric) {
    read_data <-
      convert_to_numeric_if_all_are_numeric_for_a_dataset(read_data)
  }
  return(read_data)
}

# output to csv
output_to_csv <- function(data, file_name, timestamp = T, col_names = T) {
  import_package(data.table)
  if(missing(file_name)) {
  ...
  timestamp_portion <-
    ifelse(timestamp,
           paste0("_", tolower(format(Sys.time(), "%b_%d_%Y_%H%M%S"))),
           "")
  fwrite(
    data,
    file = paste0(
...
# paste strings for message
paste_message <- function(...) {
  arguments <- list(...)
  message(paste0(arguments))
}

shiny_analysis_tool <- function(
  data_for_shiny = NULL,
  pc_port_number = NULL,
  unix_port_number = NULL,
  num_of_sigfigs = 3,
  shiny_list_max = 5000,
  shiny_saved_analysis_csv_file_name = "shiny_saved_analysis.csv",
  shiny_run_analysis_csv_file_name = "shiny_run_analysis.csv"
) {
  # import packages
  ...
  shiny_saved_analysis <-
    import_data(shiny_saved_analysis_csv_file_name,
                open_file_if_missing = F)

  # open run analysis csv
  shiny_run_analysis <-
    import_data(shiny_run_analysis_csv_file_name,
                open_file_if_missing = F)

  # create the saved analysis csv if it doesn't exist
  if(is.null(shiny_saved_analysis)) {
    ...
    output_to_csv(
      shiny_saved_analysis,
      gsub(".csv", "", shiny_saved_analysis_csv_file_name),
      timestamp = F)
    paste_message("The following file was created: ",
                  shiny_saved_analysis_csv_file_name)
  }

  # create the run analysis csv if it doesn't exist
  if(is.null(shiny_run_analysis)) {
    # initialize the csv for saving analysis
    shiny_run_analysis <-
      data.table(
        id = numeric(),
    ...
    output_to_csv(
      shiny_run_analysis,
      gsub(".csv", "", shiny_run_analysis_csv_file_name),
      timestamp = F)
    paste_message("The following file was created: ",
                  shiny_run_analysis_csv_file_name)
  }
  ...
  server <- function(input, output, session) {
    ...
    reactive_dt$saved_analysis <- import_data(
      shiny_saved_analysis_csv_file_name, open_file_if_missing = F)
    reactive_dt$run_analysis <- import_data(
      shiny_run_analysis_csv_file_name, open_file_if_missing = F)
    ...
    observeEvent(input$delete_saved_btn, {
      ...
      shiny_saved_analysis <- reactive_dt$saved_analysis
      # get id of the saved analysis
      id_of_saved_analysis <-
        gsub("(^\\d+) - .*", "\\1", input$saved_analysis_choices)
      # get the dt for saved analysis
      shiny_saved_analysis <-
        shiny_saved_analysis[!id %in% id_of_saved_analysis]
      if(nrow(shiny_saved_analysis) == 0) {
        shiny_saved_analysis <-
          data.table(
            id = numeric(),
            time = character(),
        ...
        # update id numbers
        shiny_saved_analysis[, id := match(id, unique(id))]
      }
      # save to csv
      output_to_csv(
        shiny_saved_analysis,
      ...
      reactive_dt$saved_analysis <- shiny_saved_analysis
    })
    # run analysis
...
shiny_analysis_tool(data = mtcars)
